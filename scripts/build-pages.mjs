...
const basePath = cloudflare ? '' : '/shelz-media';
const origin = cloudflare ? 'https://shelzmedia.nz' : 'https://shankarappan.github.io' + basePath;
...
// Remove stale Workers configuration generated by earlier server builds.
...
if (basePath) await rename(path.join(output, basePath, '_next'), path.join(output, '_next'));
...
console.log(
  'Static hosting artifact ready: dist/client (home, portfolio, images and metadata checked).',
);
